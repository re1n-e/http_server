...
const getChirpOwnerId = `-- name: GetChirpOwnerId :one
SELECT user_id FROM chirp WHERE id = $1
`

func (q *Queries) GetChirpOwnerId(ctx context.Context, id uuid.UUID) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, getChirpOwnerId, id)
...
const getChirpsByOwnerId = `-- name: GetChirpsByOwnerId :many
SELECT id, created_at, updated_at, body, user_id FROM chirp WHERE user_id = $1 ORDER BY created_at
`

func (q *Queries) GetChirpsByOwnerId(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsByOwnerId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
